...
def rxp_backbone(start):
    guides = {}
    for fn in glob.glob(os.path.join(RXP, 'Horde-*.lua')):
    ...
    first = {'durotar': '1-6 Durotar', 'tirisfal': '1-6 Tirisfal Glades', 'mulgore': '1-6 Mulgore'}[start]
    order, cur = [], first
    while cur and cur in guides and cur not in order:
        order.append(cur)
        nxts = [n.split('<<')[0].strip() for n in guides[cur]['next']]
        # take the first option that exists and is not class-gated for our class
        cur = None
        for raw in guides[cur if False else order[-1]]['next']:
...
for s in sim.steps:
    if s.get('q'):
        qid = s['q']
        names[qid] = Q[qid]['n'] if qid in Q else (WHQ.get(qid, {}).get('n') or '#%d' % qid)
        s['n'] = names[qid]
        if qid in Q:
...
if os.path.isdir(ADDON_DIR):
    ...
    lines = ['-- GENERATED by forever-ref/route.py; do not edit. Start %s, class %s.' % (START, CLASS), 'AlcRouteData = {', '  meta = { start = %s, cls = %s, final = %d },' % (lstr(START), lstr(CLASS), sim.level), '  dungeons = {']
    for p in packages:
    ...
    lines.append('  },')
    lines.append('  steps = {')
    # quests that must not be auto-skipped in game: dungeon packages, their prerequisites, and anything a later route quest depends on
    required = set()
    for p in packages:
        required.update(p['own']); required.update(p['pre']); required.update(p['later'])
    frontier = list(required)   # transitive closure: anything a required quest depends on is required too
    while frontier:
    ...
    for s in sim.steps:
        ...
        if pos and pos[0] and pos[0] > 0 and AREA_TO_MAP.get(pos[0]) and pos[1] is not None:
            parts.append('m = %d' % AREA_TO_MAP[pos[0]]); parts.append('x = %.1f' % pos[1]); parts.append('y = %.1f' % pos[2]); parts.append('z = %s' % lstr(ZNAME.get(pos[0], '')))
